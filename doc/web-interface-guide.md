...
右侧悬浮面板提供企业级辅助功能，可通过垂直拖动调整位置。

### 7.1 案例库

**功能说明**：
案例库提供预置文档和示例的快速访问，方便用户参考和学习。

**使用方式**：
1. 点击右侧悬浮面板中的 **"案例库"** 按钮（书本图标）
2. 弹出案例库对话框，左侧显示文件目录树
3. 点击文件夹可进入子目录，顶部显示当前路径和返回按钮
4. 点击文件查看内容，Markdown 文件自动渲染，其他文件显示原始文本

**界面功能**：
- **目录导航**：支持文件夹浏览、返回上一级
- **文件预览**：支持 Markdown 渲染和文本查看
- **全屏模式**：点击标题栏最大化按钮可全屏查看
- **拖动功能**：拖动标题栏可移动弹窗位置
- **软链接支持**：自动识别并处理软链接

**配置要求**：
案例库功能需要管理员配置 `VITE_CASE_LIBRARY_DIR` 环境变量指定文档库路径。未配置时不显示此按钮。

### 7.2 联系管理员

**功能说明**：
提供管理员联系方式，帮助用户快速获取帮助和支持。

**使用方式**：
1. 点击右侧悬浮面板中的 **"联系管理员"** 按钮（蓝色对话图标）
2. 弹出卡片显示管理员联系信息
3. 点击具体联系方式可直接复制或联系

**配置要求**：
联系管理员功能需要管理员配置 `VITE_CONTACT_ADMIN` 环境变量，格式为 `姓名:联系方式`，多个联系人用 `|` 分隔。未配置时不显示此按钮。

### 7.3 意见反馈

**功能说明**：
提供意见反馈入口，方便用户提交使用建议和问题反馈。

**使用方式**：
1. 点击右侧悬浮面板中的 **"意见反馈"** 按钮（绿色对话框图标）
2. 自动跳转到管理员配置的反馈页面

**配置要求**：
意见反馈功能需要管理员配置 `VITE_FEEDBACK_URL` 环境变量指定反馈页面地址。按钮始终显示，未配置时点击不会跳转。
